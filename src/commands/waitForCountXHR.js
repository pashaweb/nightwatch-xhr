...
WaitForXHR.prototype.poll = function () {
    ...
    this.api.execute(clientPoll, [], function ({value: xhrs}: { value: ?Array<ListenedXHR> }) {
        if (xhrs && xhrs.length) {
            ...
            if (filtered && filtered.length && filtered.length === command.count) {
...
WaitForXHR.prototype.command = function (
    ...
    this.timeout = setTimeout(function () {
        ...
        command.api.execute(clientPoll, [], function ({value: xhrs}: { value: ?Array<ListenedXHR> }) {
            if (xhrs && xhrs.length) {
                command.filtered = xhrs.filter(xhr => xhr.url.match(command.urlPattern));
            }

        });

        command.callback(command.filtered);
        // his.fail({value:false}, 'not found', this.expectedValue, defaultMsg);
        //command.client.assertion(false, 'Timed out', 'XHR Request', `Timed out waiting for ${urlPattern} XHR  ${count} times!`);
        command.emit('complete');
    }, timeout);
